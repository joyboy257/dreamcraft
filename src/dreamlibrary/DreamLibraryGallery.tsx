...
const references = [
  ["Moonlit kitchen", "Giant porcelain cup · sugar bowl · moth guide · moon window", "#a5d7ef"],
  ["Flooded school", "Waterline · lockers · desks · paper boats · childhood dog", "#3b91ca"],
  ["Lottery finale", "Jackpot board · family · instruments · banners · golden rain", "#f3ba57"],
] as const;
...
export function DreamLibraryGallery(): React.JSX.Element {
  ...
  return <main className="dreamlibrary-gallery" data-testid="dreamlibrary-gallery">
    ...
    <section aria-labelledby="materials-heading"><h2 id="materials-heading">Materials and elements</h2><div className="dreamlibrary-material-grid">{materials.map((item, index) => <article key={item.id} className="dreamlibrary-material"><span aria-hidden="true" style={{ backgroundPosition: `${(index % 16) * -32}px ${Math.floor(index / 16) * -32}px` }} /><strong>{item.label}</strong><small>{item.mobileSafe ? "mobile-safe" : "desktop"}</small></article>)}</div></section>
    <section aria-labelledby="references-heading"><h2 id="references-heading">Reference dream compositions</h2><div className="dreamlibrary-reference-grid">{references.map(([title, detail, color]) => <article key={title} className="dreamlibrary-reference" style={{ "--dream-color": color } as React.CSSProperties}><div className="dreamlibrary-scene-icon" aria-hidden="true"><span /><span /><span /></div><h3>{title}</h3><p>{detail}</p><small>Environment · entities · differentiated gameplay · ending</small></article>)}</div></section>
